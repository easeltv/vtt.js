/* vtt.js - v0.12.1 (https://github.com/mozilla/vtt.js) built on 19-09-2016 */
...
(function(global) {
  ...
  WebVTT.processCues = function(window, cues, overlay) {
    ...
    if ( cues.length > 1 ) {
      // [TH] hack to fix a bug (if successive cues have the same startTime 
      // they are displayed in reverse order). There's probably a better fix, 
      ...
      for ( var i=0, j=0; i < cues.length; i++ ) {
        ...
        if ( reorderedCues.length === 0 ) {
          reorderedCues.push( curCue );
        } else {
          inserted = false;
          for ( j=0; j < reorderedCues.length; j++ ) {
            if ( reorderedCues[j].startTime === curCue.startTime ) {
              if ( j === 0 ) {
                reorderedCues.unshift( curCue );
              } else {
                reorderedCues.splice( j - 1, 0, curCue );
              }
              inserted = true;
              break;
            }
